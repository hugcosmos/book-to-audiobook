    ...
    async def _ws_synthesize_to_file(self, text: str, output_path: Path) -> None:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # Guard against code==0 but empty audio frames — would otherwise
                # produce a 0-byte MP3 silently treated as a successful chapter.
                if not audio_frames or output_path.stat().st_size == 0:
                    raise RuntimeError("iFlytek returned no audio data")
                return
